...
Node * copyList(Node * list) {
  ...
  if(list == NULL) {
    ...
    //what is the ruuning time for copyList(suppose the linked list has N items)? 
    //The running time of the function "copyList" is O(n), where n is the number of items in the linked list. 
    //The function is visiting each node in the original list exactly oncefor each node and resulting in a running time of O(n).
  }
}
