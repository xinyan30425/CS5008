...
#define EXPERIMENT 0 // use this for the experimentation in this assignment
...
void sortIntegers(int* array, unsigned int size){
    ...
    int i,j,key; //key temperarily store the value of current element 
    for(i =1; i<size; i++){
      key = array[i];
      j=i-1;

      //whether the element to the left of i is greater than the current element
      while(j >=0 & array[j] >key){
        array[j+1] = array[j];//move the left element one position to the right
        j=j-1; //then check next element
      }
      array[j+1]=key;//insert the current element
    }
}
